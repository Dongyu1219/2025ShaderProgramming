...
void Renderer::CreateVertexBufferObjects()
{
	...
	//BufferObject를 생성하고 Object Id를 &VBO에 넣어준다. 아직 GPU에 메모리 할당 안됨.
	glGenBuffers(1, &m_VBORect);	
	glBindBuffer(GL_ARRAY_BUFFER, m_VBORect);	//바인드 해준다 : GL_ARRAY_BUFFER라는 간판을 가진 특별한 역할을 하는
	//방에 test ID를 넣어준다. ==> 작업할 대상의 형태화 용도를 구체화 해준다. 
	glBufferData(GL_ARRAY_BUFFER, sizeof(rect), rect, GL_STATIC_DRAW); // 이제 사이즈를 알고 Poineter로 전달 가능. 
	// 이줄 지나면 GPU에 메모리가 업로드 된다. 
